...
import { Mic, MicOff, Send, Play, Undo, Check, X, FileCode2, Terminal, RefreshCw, Sparkles, Cpu, Globe, Download, Search, ChevronRight, ChevronDown, GitBranch, GitCommitVertical, Save, Code2, AlignLeft, Upload, Wand2 } from 'lucide-react';
...
interface AgentEvent {
  type: 'status' | 'agent_response' | 'tool_call' | 'file_updated' | 'error' | 'files' | 'tool_result' | 'sync_state';
  payload?: any;
  callId?: string;
  result?: any;
}
...
const readFileTool: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      path: {
        type: Type.STRING,
        description: 'The path of the file to read relative to project root (e.g., src/generated/App.tsx)',
      },
    },
    required: ['path'],
  },
...
function AgentUI() {
  ...
  const [syncStatus, setSyncStatus] = useState<{ path: string, status: 'syncing' | 'success' | 'error' } | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFile, setSelectedFile] = useState<string | null>(null);
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [gitInfo, setGitInfo] = useState<{ branch: string, status: string }>({ branch: 'main', status: '' });
  ...
  const [snippets, setSnippets] = useState([
    ...
    { id: '2', name: 'useEffect Hook', code: "useEffect(() => {\n  \n  return () => {\n    \n  };\n}, []);" },
    { id: '3', name: 'Tailwind Button', code: "<button className=\"px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-black rounded-lg transition-all\">\n  Click Me\n</button>" }
  ]);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'file_updated') {
          setFiles(prev => ({ ...prev, [data.payload.path]: data.payload.content }));
          ...
          if (data.payload.type === 'transcript') setTranscript(data.payload.payload);
          else if (data.payload.type === 'status') setStatus(data.payload.payload);
          else if (data.payload.type === 'agent_response') setAgentResponses(prev => [...prev, data.payload.payload]);
          ...
          else if (data.payload.type === 'agent_response_clear') setAgentResponses([]);
          else if (data.payload.type === 'tool_call') setToolCalls(prev => [...prev, data.payload.payload]);
          else if (data.payload.type === 'tool_call_clear') setToolCalls([]);
          else if (data.payload.type === 'tool_call_result') {
            setToolCalls(prev => prev.map(c => c.name === data.payload.payload.name && JSON.stringify(c.args) === JSON.stringify(data.payload.payload.args) ? { ...c, result: data.payload.payload.result } : c));
          }
          else if (data.payload.type === 'activeTab') setActiveTab(data.payload.payload);
          else if (data.payload.type === 'isRecording') setIsRecording(data.payload.payload);
        }
      } catch (e) {
  ...
  const broadcastState = (type: string, payload: any) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: 'broadcast', payload: { type: 'sync_state', payload: { type, payload } } }));
    }
  };
  ...
  const handleFormat = async () => {
    ...
    try {
      ...
      const plugins = [prettierPluginEstree, prettierPluginTypescript, prettierPluginBabel, prettierPluginPostcss];
  ...
  const handleAIGenerate = async (description: string) => {
    ...
    try {
      ...
      const result = await ai.models.generateContent({
        ...
        contents: [{ role: 'user', parts: [{ text: `Generate code based on this description: ${description}` }] }],
      });
      const generatedCode = result.text.replace(/^```[a-z]*\n/i, '').replace(/\n```$/i, '');
      setFiles(prev => ({ ...prev, [selectedFile]: (prev[selectedFile] || '') + '\n' + generatedCode }));
...
${files[selectedFile]}`;
      ...
      const refactoredCode = result.text.replace(/^```[a-z]*\n/i, '').replace(/\n```$/i, '');
      
      if (window.confirm('AI has generated a refactored version. Apply changes?')) {
        setFiles(prev => ({ ...prev, [selectedFile]: refactoredCode }));
  ...
  const executeTool = (name: string, args: any): Promise<any> => {
    return new Promise((resolve) => {
      ...
      wsRef.current?.send(JSON.stringify({ type: 'execute_tool', callId, name, args }));
    });
  };
...
When you are done, summarize what changed.`;
      ...
      const tools = [{ functionDeclarations: [readFileTool, editFileTool, runShellTool, gitStatusTool, gitAddTool, gitCommitTool, gitPullTool, gitPushTool] }];
      ...
      while (loopCount < 10) {
        ...
        if (currentResponseText) {
          modelParts.push({ text: currentResponseText });
        }
        if (currentFunctionCalls.length > 0) {
          modelParts.push(...currentFunctionCalls.map(call => ({ functionCall: call })));
        }
        ...
        for (const call of currentFunctionCalls) {
          setToolCalls(prev => [...prev, { name: call.name, args: call.args }]);
          ...
          setToolCalls(prev => prev.map(c => c.name === call.name && JSON.stringify(c.args) === JSON.stringify(call.args) ? { ...c, result } : c));
          broadcastState('tool_call_result', { name: call.name, args: call.args, result });
  ...
  return (
    <div className="flex flex-col h-screen bg-[#030712] text-cyan-50 font-sans w-full max-w-md mx-auto border-x border-cyan-900/30 shadow-[0_0_50px_rgba(8,145,178,0.1)] overflow-hidden relative">
          ...
          >
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-light text-white tracking-widest uppercase">Snippet Library</h2>
                <button onClick={() => setIsSnippetManagerOpen(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <input 
                type="text" 
              ...
              <div className="flex-1 overflow-y-auto space-y-2">
                {snippets.map(s => (
                  <div key={s.id} className="bg-gray-800 p-3 rounded-lg flex justify-between items-center">
                    <span className="text-cyan-300 font-mono text-sm">{s.name}</span>
                    <div className="flex gap-2">
                      ...
                      }} className="text-xs bg-cyan-500/20 text-cyan-300 px-2 py-1 rounded">Insert</button>
                      <button onClick={() => setSnippets(prev => prev.filter(p => p.id !== s.id))} className="text-xs bg-red-500/20 text-red-300 px-2 py-1 rounded">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          >
            ...
            <div className="w-full max-w-xs space-y-3 z-10">
              {BOOT_SEQUENCE.map((step, index) => (
                ...
                >
                  <div className={`w-1.5 h-1.5 rounded-full ${index < bootStep ? 'bg-cyan-400 shadow-[0_0_8px_#22d3ee]' : index === bootStep ? 'bg-indigo-400 animate-pulse' : 'bg-gray-800'}`}></div>
                  <span className={index < bootStep ? 'text-cyan-300' : index === bootStep ? 'text-indigo-300' : 'text-gray-600'}>
                    {step}
                  </span>
                </motion.div>
              ))}
            ...
            <div className="absolute bottom-10 left-0 right-0 flex justify-center z-10">
              <div className="w-48 h-1 bg-gray-900 rounded-full overflow-hidden">
                <motion.div 
                  ...
                  initial={{ width: '0%' }}
                  animate={{ width: `${(Math.min(bootStep, BOOT_SEQUENCE.length) / BOOT_SEQUENCE.length) * 100}%` }}
      ...
      {/* Header */}
      <header className="bg-black/40 backdrop-blur-xl border-b border-cyan-500/20 p-4 flex items-center justify-between z-10 relative">
        <div className="flex items-center gap-3">
          <div className="relative w-10 h-10 rounded-full bg-cyan-950 border border-cyan-500/50 flex items-center justify-center shadow-[0_0_15px_rgba(6,182,212,0.4)]">
            <Cpu className="w-5 h-5 text-cyan-400" />
            <div className="absolute inset-0 rounded-full border border-cyan-400/30 animate-[spin_4s_linear_infinite]"></div>
          </div>
          <div>
            <h1 className="font-bold text-lg tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-400">Voice PM</h1>
            <div className="text-[9px] font-mono text-cyan-500/80 tracking-widest uppercase">V2.0 // Neural Link</div>
          </div>
        </div>
        <div className="flex flex-col items-end gap-1">
          <div className="text-[10px] font-mono text-cyan-300 bg-cyan-950/50 border border-cyan-500/30 px-3 py-1.5 rounded-full shadow-[0_0_10px_rgba(6,182,212,0.2)]">
            {status}
          </div>
          <AnimatePresence>
            {syncStatus && (
              ...
              >
                <Check className="w-3 h-3" /> SYNCED: {syncStatus.path.split('/').pop()}
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <main className="flex-1 flex overflow-hidden relative z-10">
        ...
        <AnimatePresence>
          {isSidebarOpen && (
            <motion.aside
              ...
              className="bg-black/60 backdrop-blur-xl border-r border-cyan-900/30 flex flex-col overflow-hidden"
            >
              <div className="p-4 space-y-4 flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide space-y-6">
                  <div>
                    <div className="flex items-center gap-2 mb-2 px-1">
                      ...
                      <span className="text-[9px] font-mono text-cyan-500 uppercase tracking-widest">Project Files</span>
                    </div>
                    <FileTree 
                      files={files} 
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-2 px-1">
                      ...
                      <span className="text-[9px] font-mono text-indigo-400 uppercase tracking-widest">Snippets</span>
                    </div>
                      ...
                      {snippets.map(snippet => (
                        <button
                          ...
                          onClick={() => {
                            if (selectedFile) {
                              setFiles(prev => ({ ...prev, [selectedFile]: (prev[selectedFile] || '') + '\n' + snippet.code }));
                              setStatusAndBroadcast(`INSERTED: ${snippet.name}`);
                            }
                          }}
                ...
                <div className="pt-4 border-t border-cyan-900/30 space-y-3">
                  <div className="flex items-center justify-between px-1">
                    <span className="text-[9px] font-mono text-cyan-700 uppercase tracking-widest">Autosave</span>
                    <button 
                      onClick={() => setIsAutosaveEnabled(!isAutosaveEnabled)}
                      className={`w-8 h-4 rounded-full relative transition-colors ${isAutosaveEnabled ? 'bg-cyan-500' : 'bg-gray-800'}`}
                    >
                      <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${isAutosaveEnabled ? 'left-4.5' : 'left-0.5'}`}></div>
                    </button>
                  </div>
                  <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-2">
                      <GitBranch className="w-3 h-3 text-indigo-400" />
                      <span className="text-[9px] font-mono text-indigo-400 uppercase tracking-widest">{gitInfo.branch}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <button onClick={() => {
                        executeTool('gitPull', {}).then(res => setStatusAndBroadcast(res.error ? `PULL ERR: ${res.error}` : 'PULLED'));
                      }} className="p-1 hover:text-indigo-300 transition-colors">
                        <Download className="w-2.5 h-2.5" />
                      </button>
                      <button onClick={() => {
                        executeTool('gitPush', {}).then(res => setStatusAndBroadcast(res.error ? `PUSH ERR: ${res.error}` : 'PUSHED'));
                      }} className="p-1 hover:text-indigo-300 transition-colors">
                        <Upload className="w-2.5 h-2.5" />
                      </button>
                      <button onClick={() => {
                        executeTool('gitStatus', {}).then(res => setGitInfo(prev => ({ ...prev, status: res.output })));
                      }} className="p-1 hover:text-indigo-300 transition-colors">
        ...
        <div className="flex-1 flex flex-col overflow-hidden p-5 pb-28">
          <button 
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-30 bg-cyan-950/80 border border-cyan-500/30 rounded-r-lg p-1 text-cyan-400 hover:text-white transition-colors"
          >
            {isSidebarOpen ? <ChevronRight className="w-3 h-3 rotate-180" /> : <ChevronRight className="w-3 h-3" />}
          </button>
          ...
          {activeTab === 'Talk' && (
            <motion.div 
              ...
              className="flex flex-col h-full overflow-y-auto scrollbar-hide p-4 gap-10"
            >
              <div className="text-center space-y-3">
                <h2 className="text-3xl font-light tracking-tight text-white">Awaiting Directive</h2>
                <p className="text-cyan-500/70 text-sm font-mono uppercase tracking-widest">Initialize Voice Protocol</p>
              </div>
              ...
              <div className="relative group">
                ...
                >
                  ...
                  {isRecording ? (
                    ...
                    <MicOff className="w-12 h-12 text-cyan-800" />
                  )}
                </button>
              </div>

              <div className="w-full max-w-sm">
                <div className="bg-black/40 backdrop-blur-md border border-cyan-900/50 rounded-2xl p-5 min-h-[140px] relative shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]">
                  <div className="absolute top-0 left-4 -translate-y-1/2 bg-[#030712] px-2 text-[10px] font-mono text-cyan-600 tracking-widest">TRANSCRIPT</div>
                  <textarea
                    className="w-full bg-transparent text-cyan-100 font-light leading-relaxed resize-none focus:outline-none placeholder-cyan-900/60 h-full min-h-[100px]"
                    placeholder='"Design a holographic dashboard..."'
                    value={transcript}
                    onChange={(e) => setTranscriptAndBroadcast(e.target.value)}
                  />
                  ...
                  {transcript && !isRecording && (
                    <motion.button 
                      ...
                      onClick={sendPrompt}
                      className="absolute -bottom-5 right-5 bg-cyan-500 hover:bg-cyan-400 text-black p-4 rounded-full shadow-[0_0_20px_rgba(6,182,212,0.5)] transition-all hover:scale-110"
                    >
          ...
          {activeTab === 'Review' && (
            <motion.div 
              ...
              className="flex-1 overflow-y-auto scrollbar-hide space-y-8 pb-20 p-4"
            >
              <div className="space-y-3">
                <div className="flex items-center gap-2 border-b border-cyan-900/50 pb-2">
                  <Globe className="w-4 h-4 text-cyan-500" />
                  <h3 className="text-[11px] font-mono text-cyan-400 uppercase tracking-widest">Neural Link Stream</h3>
                </div>
                    ...
                    {agentResponses.map((res, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] font-mono text-cyan-500 uppercase tracking-widest">Thought Process</span>
                        </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 border-b border-cyan-900/50 pb-2">
                  ...
                  <h3 className="text-[11px] font-mono text-indigo-400 uppercase tracking-widest">System Operations</h3>
                </div>
                {toolCalls.length === 0 ? (
                  <div className="text-cyan-900/50 text-xs font-mono italic">No operations executed.</div>
                ) : (
                  <div className="space-y-3">
                    {toolCalls.map((call, i) => (
                      ...
                      >
                        ...
                        {call.name === 'readFile' || call.name === 'editFile' ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <div className="text-xs font-bold text-indigo-300 uppercase tracking-wider">{call.name}</div>
                            {!call.result && (
                          ...
                          {call.result && (
                            ...
                            >
                              <div className="flex items-center gap-2 mb-1 border-b border-green-900/30 pb-1">
                                ...
                                <span className="text-green-500 font-bold uppercase tracking-widest">Operation Successful</span>
                              </div>
          ...
          {activeTab === 'Apply' && (
            ...
            >
              <div className="flex-1 flex flex-col bg-black/40 backdrop-blur-md border border-cyan-900/30 rounded-2xl overflow-hidden shadow-2xl">
                <div className="bg-cyan-950/50 px-4 py-3 border-b border-cyan-900/50 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-cyan-500 animate-pulse shadow-[0_0_8px_rgba(6,182,212,0.8)]"></div>
                    <span className="text-[10px] font-mono text-cyan-300 uppercase tracking-widest">{selectedFile || 'Select a sector to modify'}</span>
                    {lastSaved && (
                      <span className="text-[9px] font-mono text-cyan-700 hidden sm:inline">Last Sync: {lastSaved.toLocaleTimeString()}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <button 
                      ...
                      title="AI Refactor"
                    >
                      <Sparkles className={`w-4 h-4 ${isRefactoring ? 'animate-spin' : ''}`} />
                    </button>
                    <button 
                      onClick={() => {
                        const desc = prompt('Describe the code you want to generate:');
                        if (desc) handleAIGenerate(desc);
                      }}
                    ...
                    <button 
                      onClick={() => {
                        if (selectedFile) {
                          wsRef.current?.send(JSON.stringify({ type: 'execute_tool', callId: 'manual-save-' + Date.now(), name: 'editFile', args: { path: selectedFile, content: files[selectedFile] } }));
                          setLastSaved(new Date());
                        }
                      }}
                ...
                <div className="flex-1 overflow-auto relative custom-editor">
                  {selectedFile ? (
                    <div className="flex min-h-full">
                      ...
                      <div className="flex-1">
                        <Editor
                          ...
                          onValueChange={code => setFiles(prev => ({ ...prev, [selectedFile]: code }))}
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center p-10 space-y-4">
                      ...
                      <div>
                        <h3 className="text-sm font-mono text-cyan-700 uppercase tracking-widest">No Sector Selected</h3>
                        <p className="text-[10px] font-mono text-cyan-900 mt-1">Select a file from the neural link to begin synthesis</p>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  onClick={() => {
                    if (wsRef.current?.readyState === WebSocket.OPEN) {
                      Object.keys(files).forEach(path => {
                        wsRef.current?.send(JSON.stringify({ type: 'revert_file', payload: { path } }));
                      });
            ...
            >
              <div className="bg-black/80 backdrop-blur-xl border border-cyan-900/50 rounded-t-2xl p-3 flex items-center justify-between z-10">
                ...
                <div className="text-[10px] font-mono text-cyan-400 uppercase tracking-widest flex items-center gap-2 bg-cyan-950/50 px-3 py-1 rounded-full border border-cyan-900/50">
                  <RefreshCw className="w-3 h-3 cursor-pointer hover:text-white transition-colors" onClick={() => {
              ...
              <div className="flex-1 bg-white rounded-b-2xl overflow-hidden relative flex items-center justify-center border-x border-b border-cyan-900/50 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                <iframe id="preview-iframe" src="/preview" className="w-full h-full border-0" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
...
function FileTree({ files, onSelect, selectedFile, searchQuery }: { files: Record<string, string>, onSelect: (path: string) => void, selectedFile: string | null, searchQuery: string }) {
  const filteredFiles = Object.keys(files).filter(path => path.toLowerCase().includes(searchQuery.toLowerCase()));
  
  return (
    <div className="space-y-1">
      {filteredFiles.length === 0 ? (
        <div className="px-3 py-4 text-[9px] font-mono text-cyan-900 italic">No sectors found</div>
      ) : (
...
function NavItem({ icon, label, isActive, onClick }: { icon: React.ReactNode, label: string, isActive: boolean, onClick: () => void }) {
  return (
    ...
    >
      <div className={`mb-1 ${isActive ? 'scale-110 drop-shadow-[0_0_5px_rgba(6,182,212,0.5)]' : ''}`}>
        {icon}
      </div>
